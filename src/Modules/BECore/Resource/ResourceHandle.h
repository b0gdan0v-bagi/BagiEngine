...
namespace BECore {
     ...
     */
    template <typename T>
    requires std::derived_from<T, IResource> class ResourceHandle {
    public:
         ...
         */
        explicit ResourceHandle(IntrusivePtr<T> resource) : _resource(std::move(resource)), _lastAccess(std::chrono::steady_clock::now()) {}
