...
namespace BECore {
    ...
    struct DefaultChecker<T, std::enable_if_t<std::is_arithmetic_v<T>>> {
        ...
        static bool IsDefault(const T& v) {
            return v == T{};
        }
    };
    ...
    template <typename T>
        struct DefaultChecker < T,
        std::enable_if_t < !std::is_arithmetic_v<T> && !std::is_enum_v<T> && requires(const T& t) {
        {
            t.empty()
    ...
    struct DefaultChecker<PoolString> {
        static constexpr bool AlwaysFalse = false;
        static bool IsDefault(const PoolString& v) {
            return v.Empty();
        }
    };
    ...
    struct DefaultChecker<IntrusivePtrAtomic<T>> {
        static constexpr bool AlwaysFalse = false;
        static bool IsDefault(const IntrusivePtrAtomic<T>& v) {
            return !v;
        }
    };
    ...
    struct DefaultChecker<IntrusivePtrNonAtomic<T>> {
        static constexpr bool AlwaysFalse = false;
        static bool IsDefault(const IntrusivePtrNonAtomic<T>& v) {
            return !v;
        }
    };
    ...
    struct DefaultChecker<eastl::optional<T>> {
        static constexpr bool AlwaysFalse = false;
        static bool IsDefault(const eastl::optional<T>& v) {
            return !v.has_value();
        }
    };
    ...
    struct DefaultChecker<std::optional<T>> {
        static constexpr bool AlwaysFalse = false;
        static bool IsDefault(const std::optional<T>& v) {
            return !v.has_value();
        }
    };
    ...
    struct DefaultChecker<eastl::pair<A, B>> {
        static constexpr bool AlwaysFalse = DefaultChecker<A>::AlwaysFalse || DefaultChecker<B>::AlwaysFalse;
        static bool IsDefault(const eastl::pair<A, B>& v) {
            return DefaultChecker<A>::IsDefault(v.first) && DefaultChecker<B>::IsDefault(v.second);
        }
    };
    ...
    template <typename T>
        struct DefaultMaker < T,
        std::enable_if_t < !std::is_arithmetic_v<T> && !std::is_enum_v<T> && requires(T & t) {
        t.clear();
    } >> {
        static constexpr bool HasMakeDefault = true;
        static void MakeDefault(T & v) {
    ...
    struct DefaultMaker<PoolString> {
        static constexpr bool HasMakeDefault = true;
        static void MakeDefault(PoolString& v) {
            v = PoolString{};
        }
    };
    ...
    struct DefaultMaker<IntrusivePtrAtomic<T>> {
        static constexpr bool HasMakeDefault = true;
        static void MakeDefault(IntrusivePtrAtomic<T>& v) {
            v.Reset();
        }
    };
    ...
    struct DefaultMaker<IntrusivePtrNonAtomic<T>> {
        static constexpr bool HasMakeDefault = true;
        static void MakeDefault(IntrusivePtrNonAtomic<T>& v) {
            v.Reset();
        }
    };
    ...
    struct DefaultMaker<eastl::optional<T>> {
        static constexpr bool HasMakeDefault = true;
        static void MakeDefault(eastl::optional<T>& v) {
            v = eastl::nullopt;
        }
    };
