...
namespace eastl {
    ...
    template <BECore::Details::FixedString Str>
    struct hash<BECore::StaticPoolString<Str>> {
        constexpr size_t operator()(const BECore::StaticPoolString<Str>& sps) const noexcept {
            return static_cast<size_t>(sps.HashValue());
        }
    };

    template <>
    struct hash<BECore::PoolString> {
        size_t operator()(const BECore::PoolString& ps) const noexcept {
...
namespace eastl {
    ...
    struct equal_to<BECore::PoolString> {
        ...
        constexpr bool operator()(const BECore::PoolString& lhs, const eastl::string_view& rhs) const noexcept {
            return lhs.ToStringView() == rhs;
        }
        constexpr bool operator()(const eastl::string_view& lhs, const BECore::PoolString& rhs) const noexcept {
            return lhs == rhs.ToStringView();
        }
    };

    template <BECore::Details::FixedString Str>
    struct equal_to<BECore::StaticPoolString<Str>> {
        template <BECore::Details::FixedString OtherStr>
        constexpr bool operator()(const BECore::StaticPoolString<Str>& lhs, const BECore::StaticPoolString<OtherStr>& rhs) const noexcept {
            return lhs == rhs;
        }

        constexpr bool operator()(const BECore::StaticPoolString<Str>& lhs, const BECore::PoolString& rhs) const noexcept {
            return lhs == rhs;
        }

        constexpr bool operator()(const BECore::PoolString& lhs, const BECore::StaticPoolString<Str>& rhs) const noexcept {
            return rhs == lhs;
        }
    };
